...
if __name__=='__main__':
    ...
    for k in range(0,40):
        ...
        img_all_obj_file_path=folders_composite[k]+"all_object.tif"
        img_all_obj=io.imread(img_all_obj_file_path)
        ...
        for i in range(0,1031,2):
            ...
            for j in range(0,1231,2):
                if (img_all_obj[i:i+50,j:j+50]==0).sum()<=50 or (img_all_obj[i:i+50,j:j+50]==1).sum()<=30:
                    continue
                cur_co_df=GetCoordinates(img,img_all_obj,i,i+50,j,j+50)
                if cur_co_df is not None:
                    obj_df=obj_df.append(cur_co_df).reset_index(drop=True)
